...
**핵심 원칙: 위 Skill들의 규칙을 반드시 준수하며, 테스트가 먼저, 구현은 나중 (TDD Red 단계)**
...
# 기존 테스트 구조 확인
ls tests/modules/*/
head -50 tests/modules/*/test_*.py  # 파일 구조, fixture, mock 패턴 확인
```
...
5. `test_{feature}.py` 생성 (기능별 테스트, vertical-slice-rules 준수)

### Step 5: pytest 실행 (FAILED 확인)
...
from src.modules.{domain}._models import {Domain}
...
2. **소스 코드 참조 금지** - 아직 존재하지 않음
3. **SPEC만으로 테스트 작성** - SPEC이 진실의 원천
4. **Import는 예상 경로로** - 나중에 logic-code-generator가 맞춤
5. **테스트가 PASSED면 안 됨** - 구현 전이므로 반드시 실패해야 함
6. **기능별 파일 분리** - test_{feature}.py 형식
7. **패턴 일관성** - 기존 테스트와 동일한 패턴 사용
